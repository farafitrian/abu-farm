...
app.post('/api/auth/register', (req, res) => {
  const { name, email, password, phone, consent } = req.body;
  ...
  if (phone && (phoneDigits.length < 10 || phoneDigits.length > 13)) return res.status(400).json({ error: 'Nomor HP harus 10-13 digit' });
  if (!consent) return res.status(400).json({ error: 'Persetujuan diperlukan' });
  try {
    run("INSERT INTO users (name,email,password,phone) VALUES (?,?,?,?)", [name, email, bcrypt.hashSync(password, 10), phoneDigits || '']);
...
function autoReply(msg, stok) {
  ...
  if (msg.includes('vaksin') || msg.includes('sehat')) return 'Semua hewan sudah vaksinasi lengkap (Anthrax, PMK). Hubungi kami untuk info kesehatan hewan.';
